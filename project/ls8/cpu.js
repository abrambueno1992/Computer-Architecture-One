...
class CPU {
    ...
    constructor(ram) {
        ...
        this.IM = 5; // R5, interrupt mask 11111111 /8; 11110000/4 X.X.X.A/class C network
    ...
    alu(op, regA, regB) {
        switch (op) {
            ...
            case PUSH:
                ...
                // console.log('reg', this.reg)
                this.reg[this.SP] -= 1;
                this.ram.write(this.reg[this.SP], this.reg[regA])
                this.PC += 2;
                // console.log('reg', this.reg)
            ...
            case ST:
                ...
                break;
            case JMP:
                this.PC = this.ram.read(this.reg[regA]);
                // console.log('jump infinite')
                break;
            case PRA:
                ...
                break;
            case CMP:
                // `FL` bits: `00000LGE` ; it's internal
                ...
                this.PC += 3;
                // console.log('comparison, FL: ', this.FL)
                break;
            case JEQ:
                // console.log('JEQ, before this.PC', this.PC)
                if (this.FL === 0b00000001) {
                    this.PC = this.reg[regA];
                } else {
                    this.PC += 2;
                }
                // console.log('JEQ, after this.PC', this.PC)
                break;
            case JNE:
                // console.log('JNE, before this.PC', this.PC)
                if (this.FL !== 0b00000001) {
                    this.PC = this.reg[regA];
                } else {
                    this.PC += 2;
                }
                // console.log('JNE, after this.PC', this.PC)
                break;
            case IRET:
                // console.log('iret executed')
                break;
            default:
                this.stopClock();
                break;
